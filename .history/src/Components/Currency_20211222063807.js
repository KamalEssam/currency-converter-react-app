...
export default function Currency() {
    ...
    function format(number) {
      ...
      function handleCurrency1Change(frCurrency) {
        secAmount(format(frAmount * currencyExchange[secCurrency] / currencyExchange[frCurrency]));
        setFrAmount(frAmount);
        
      }
    
      function handleAmount2Change(secAmount) {
        frAmount(format(frAmount * currencyExchange[secCurrency] / currencyExchange[secCurrency]));
        setFrAmount(frAmount);
